    ...
    public void setTabelas() {
        ...
        for (int i = padraoLength-2; i >= 0; i--) {
            ...
            for (int j = (i + 1); j >= 0; j--) {
                ...
                // se o sufixo for igual ao auxiliar e o caractere anterior ao sufixo for diferente do caractere anterior ao padrao
                if (j != 0 && (aux.equals(sufixo) && restoPadrao.charAt(j-1) != padrao.charAt(i))) {
                    sufBom[i] = (i + 1) - j;
                    j = -1; // sai do loop
                } else if (j == 0 && aux.equals(sufixo)) { // se o sufixo for igual ao auxiliar e estivermos no primeiro char do padrao
                    sufBom[i] = (i + 1) - j;
                    j = -1; // sai do loop
                }
            }
            ...
            if (sufBom[i] == 0) {
                ...
                while (index < (padraoLength - 1)) {
                    ...
                    restoPadrao = padrao.substring(0, sufixoLength); // pega o prefixo do padrao com o mesmo tamanho do sufixo
        ...
        for (int i = 0; i < bancoLength; i++) {
            ...
            for (int j = (padraoLength - 1); j >= 0; j--) {
                if ((i + j) < (bancoLength - 1)) { // se o indice do banco nao estourar
                    if ((padrao.charAt(j) != banco.charAt(i+j))) { // se o caractere do padrao for diferente do caractere do banco
                        if (charRuim.containsKey(banco.charAt(i+j))) { // se o caractere do banco estiver na tabela charRuim
                            shift = Math.max(sufBom[j], (j - charRuim.get(banco.charAt(i+j)))); // shift recebe o maior valor entre as tabelas de sufBom e charRuim
                        } else {
                            shift = Math.max(sufBom[j], (j - (-1))); // se o caractere do banco nao estiver na tabela charRuim, usar -1 como default
                        }
                        ...
                        } else { i = bancoLength; } // sai do loop
                        j = -1; // sai do loop
                    } else if (j == 0) { // se o caractere do padrao for igual ao caractere do banco e estivermos no primeiro caractere do padrao
