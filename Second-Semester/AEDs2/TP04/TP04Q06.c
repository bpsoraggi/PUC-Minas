...
typedef struct
{
  ...
  char name[MAX_FIELD_SIZE], owners[MAX_FIELD_SIZE], website[MAX_FIELD_SIZE],
      developers[MAX_FIELD_SIZE], languages[MAX_STRING_ARRAY_SIZE][30],
...
void game_print(Game *game)
{
  ...
  int hours = game->avg_playtime / 60, minutes = game->avg_playtime % 60;

  printf("%i %s %s/%04i %s %i %.2f %i [", game->app_id, game->name,
         getMonthName(game->release_date.month), game->release_date.year,
         game->owners, game->age, game->price, game->dlcs);

  for (int i = 0; i < game->count_languages; i++)
  {

    printf("%s%s", game->languages[i],
           i < game->count_languages - 1 ? ", " : "");
  }

  printf("] %s %s %s %s ", game->website, game->windows_os ? "true" : "false",
         game->mac_os ? "true" : "false", game->linux_os ? "true" : "false");
...
int main()
{
  ...
  for (int i = 0; i < cont - 1; i++)
  {
    ...
    {
      if (jogos[menor].release_date.year > jogos[j].release_date.year)
      {
        menor = j;
      }
      else if (jogos[menor].release_date.month > jogos[j].release_date.month && jogos[menor].release_date.year == jogos[j].release_date.year)
      {
        menor = j;
      }
    }
    Game temp = jogos[menor];
    jogos[menor] = jogos[i];
    jogos[i] = temp;
  }
