class No {
    ...
    public No(GameOficial elemento, No esq, No dir, int nivel) {
...
class AVL {
    ...
    public void inserir(GameOficial x) throws Exception {
        raiz = inserir(x, raiz);
    }

    private No inserir(GameOficial x, No i) throws Exception {
        if (i == null) {
            i = new No(x);
    ...
    private No remover(String x, No i) throws Exception {
        if (i == null) {
            throw new Exception("Erro na remoção");
        } else if (x.compareTo(i.elemento.getName()) < 0) {
    ...
    private No balancear(No no) throws Exception {
        if (no != null) {
            ...
            if (Math.abs(fator) <= 1) {
                ...
                throw new Exception(
                        "Erro no Nó(" + no.elemento + ") com fator de balanceamento (" + fator + ") inválido!");
            }
        }
        return no;
    }
